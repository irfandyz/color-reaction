    ...
    <div class="row mt-5">
        ...
        <div class="col-sm-4">
            ...
            <form action="{{ asset('sign-in') }}" method="post">
                @csrf
                <input type="text" name="username" id="username" class="input-form" placeholder="username">
                @if (Session::has('message'))
                    <small class="text-warning fw-bold">{{ Session::get('message') }}</small>
                @endif
                ...
                <button type="submit" class="btn btn-success color-1 w-100 mt-4 fw-bold">Masuk</button>
            </form>
            <div class="separator mt-3 text-white"><small>ATAU</small></div>
            <p class="text-center mt-2" style="font-size:18px">
                Belum Punya Akun? Ayo <a href="{{ asset('sign-up') }}" class="text-warning fw-bold text-decoration-none">Daftar</a>
            </p>
        </div>
    </div>
